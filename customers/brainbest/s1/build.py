...
def render(s):
    ...
    <div class="step imgstep" {attrs}>
      {ext}<div class="imgcard"><img src="assets/diagrams/{s["img"]}" alt="{attr(s["cap"])}"></div>
      {scene}
    </div>'''
    if k == "close":
        if s.get("flow"):
...
EXTRA_CSS = """
    ...
    /* Los tres campos del scene box, con la misma jerarquía visual que la
       galería: titular fuerte, cuerpo gris y más chico, cierre en verde. */
    ...
    .cap-take {
      ...
      font-weight: 600;
      line-height: 1.45;
      color: #15803d;              /* verde: el cierre, igual que la galería */
    ...
    @media (max-width: 820px) {
      ...
      .cap-body { font-size: clamp(12px, 2.8vw, 15px); }
      .cap-take { font-size: clamp(13px, 3.0vw, 16px); }
